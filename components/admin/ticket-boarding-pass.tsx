...
import { Ticket, MapPin, Calendar, Clock, User, Armchair, QrCode } from "lucide-react";
...
export const TicketBoardingPass = React.forwardRef<HTMLDivElement, TicketBoardingPassProps>(
    ({ ticket }, ref) => {
        const tripDate = new Date(ticket.trips?.departure_time || new Date());
        const bookDate = new Date(ticket.created_at);
        const passengerName = ticket.passenger_name || ticket.profiles?.full_name || 'Walk-in Guest';

        return (
            <div ref={ref} className="w-full relative overflow-hidden rounded-2xl bg-white shadow-xl border border-zinc-100 dark:bg-zinc-900 dark:border-zinc-800">
                {/* Ticket Header */}
                ...
                <div className="p-6 pb-4 space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-y-4 gap-x-2 bg-zinc-50 rounded-xl p-4 border border-zinc-100 dark:bg-zinc-950 dark:border-zinc-800">
                        ...
                        <div className="space-y-0.5">
                            ...
                            <p className="font-bold text-sm truncate" title={passengerName}>
                                {passengerName}
                            </p>
                        </div>
                        <div className="space-y-0.5">
...
TicketBoardingPass.displayName = "TicketBoardingPass";
